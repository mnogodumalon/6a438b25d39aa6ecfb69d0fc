...
import type { Terminverwaltung, Kundenverwaltung, Monteurdaten, CreateTerminverwaltung, CreateKundenverwaltung, CreateMonteurdaten } from '@/types/app';
...
export class LivingAppsService {
  // --- TERMINVERWALTUNG ---
  static async getTerminverwaltung(): Promise<Terminverwaltung[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.TERMINVERWALTUNG}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Terminverwaltung[];
    return enrichLookupFields(records, 'terminverwaltung');
  }
  static async getTerminverwaltungEntry(id: string): Promise<Terminverwaltung | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.TERMINVERWALTUNG}/records/${id}`);
    const record = { record_id: data.id, ...data } as Terminverwaltung;
    return enrichLookupFields([record], 'terminverwaltung')[0];
  }
  static async createTerminverwaltungEntry(fields: CreateTerminverwaltung) {
    return callApi('POST', `/apps/${APP_IDS.TERMINVERWALTUNG}/records`, { fields: cleanFieldsForApi(fields as any, 'terminverwaltung') });
  }
  static async updateTerminverwaltungEntry(id: string, fields: Partial<CreateTerminverwaltung>) {
    return callApi('PATCH', `/apps/${APP_IDS.TERMINVERWALTUNG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'terminverwaltung') });
  }
  static async deleteTerminverwaltungEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.TERMINVERWALTUNG}/records/${id}`);
  }
  ...
  // --- MONTEURDATEN ---
  static async getMonteurdaten(): Promise<Monteurdaten[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.MONTEURDATEN}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Monteurdaten[];
    return enrichLookupFields(records, 'monteurdaten');
  }
  static async getMonteurdatenEntry(id: string): Promise<Monteurdaten | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.MONTEURDATEN}/records/${id}`);
    const record = { record_id: data.id, ...data } as Monteurdaten;
    return enrichLookupFields([record], 'monteurdaten')[0];
  }
  static async createMonteurdatenEntry(fields: CreateMonteurdaten) {
    return callApi('POST', `/apps/${APP_IDS.MONTEURDATEN}/records`, { fields: cleanFieldsForApi(fields as any, 'monteurdaten') });
  }
  static async updateMonteurdatenEntry(id: string, fields: Partial<CreateMonteurdaten>) {
    return callApi('PATCH', `/apps/${APP_IDS.MONTEURDATEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'monteurdaten') });
  }
  static async deleteMonteurdatenEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.MONTEURDATEN}/records/${id}`);
  }
