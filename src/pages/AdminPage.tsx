...
import { TerminverwaltungViewDialog } from '@/components/dialogs/TerminverwaltungViewDialog';
...
import { MonteurdatenViewDialog } from '@/components/dialogs/MonteurdatenViewDialog';
...
const TERMINVERWALTUNG_FIELDS = [
  { key: 'termin_datum', label: 'Datum und Uhrzeit', type: 'date/datetimeminute' },
  { key: 'auftragsart', label: 'Auftragsart', type: 'lookup/radio', options: [{ key: 'wartung', label: 'Wartung' }, { key: 'reparatur', label: 'Reparatur' }, { key: 'notdienst', label: 'Notdienst' }] },
  ...
  { key: 'monteur', label: 'Monteur', type: 'applookup/select', targetEntity: 'monteurdaten', targetAppId: 'MONTEURDATEN', displayField: 'monteur_vorname' },
  { key: 'beschreibung', label: 'Beschreibung / Notizen', type: 'string/textarea' },
  { key: 'erledigt', label: 'Termin erledigt', type: 'bool' },
];
...
const MONTEURDATEN_FIELDS = [
  ...
  { key: 'monteur_notiz', label: 'Notiz', type: 'string/text' },
];

const ENTITY_TABS = [
  { key: 'terminverwaltung', label: 'Terminverwaltung', pascal: 'Terminverwaltung' },
  { key: 'kundenverwaltung', label: 'Kundenverwaltung', pascal: 'Kundenverwaltung' },
  { key: 'monteurdaten', label: 'Monteurdaten', pascal: 'Monteurdaten' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('terminverwaltung');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'terminverwaltung': new Set(),
    'kundenverwaltung': new Set(),
    'monteurdaten': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'terminverwaltung': return (data as any).terminverwaltung as Terminverwaltung[] ?? [];
      case 'kundenverwaltung': return (data as any).kundenverwaltung as Kundenverwaltung[] ?? [];
      case 'monteurdaten': return (data as any).monteurdaten as Monteurdaten[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getLookupLists = useCallback((entity: EntityKey) => {
    ...
    switch (entity) {
      case 'terminverwaltung':
        ...
        lists.monteurdatenList = (data as any).monteurdaten ?? [];
        break;
    }
    return lists;
  }, [data]);
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'terminverwaltung' && fieldKey === 'kunde') {
      const match = (lists.kundenverwaltungList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.vorname ?? '—';
    }
    if (entity === 'terminverwaltung' && fieldKey === 'monteur') {
      const match = (lists.monteurdatenList ?? []).find((r: any) => r.record_id === id);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'terminverwaltung': return TERMINVERWALTUNG_FIELDS;
      case 'kundenverwaltung': return KUNDENVERWALTUNG_FIELDS;
      case 'monteurdaten': return MONTEURDATEN_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'terminverwaltung': return {
        create: (fields: any) => LivingAppsService.createTerminverwaltungEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateTerminverwaltungEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteTerminverwaltungEntry(id),
      };
      ...
      case 'monteurdaten': return {
        create: (fields: any) => LivingAppsService.createMonteurdatenEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateMonteurdatenEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteMonteurdatenEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'terminverwaltung' || dialogState?.entity === 'terminverwaltung') && (
        <TerminverwaltungDialog
          open={createEntity === 'terminverwaltung' || dialogState?.entity === 'terminverwaltung'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'terminverwaltung' ? handleUpdate : (fields: any) => handleCreate('terminverwaltung', fields)}
          defaultValues={dialogState?.entity === 'terminverwaltung' ? dialogState.record?.fields : undefined}
          kundenverwaltungList={(data as any).kundenverwaltung ?? []}
          monteurdatenList={(data as any).monteurdaten ?? []}
      ...
      {(createEntity === 'monteurdaten' || dialogState?.entity === 'monteurdaten') && (
        <MonteurdatenDialog
          open={createEntity === 'monteurdaten' || dialogState?.entity === 'monteurdaten'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'monteurdaten' ? handleUpdate : (fields: any) => handleCreate('monteurdaten', fields)}
          defaultValues={dialogState?.entity === 'monteurdaten' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Monteurdaten']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Monteurdaten']}
        />
      )}
      {viewState?.entity === 'terminverwaltung' && (
        <TerminverwaltungViewDialog
          open={viewState?.entity === 'terminverwaltung'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'terminverwaltung', record: r }); }}
          kundenverwaltungList={(data as any).kundenverwaltung ?? []}
          monteurdatenList={(data as any).monteurdaten ?? []}
        />
      )}
      ...
      {viewState?.entity === 'monteurdaten' && (
        <MonteurdatenViewDialog
          open={viewState?.entity === 'monteurdaten'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'monteurdaten', record: r }); }}
        />
      )}
